...
//functional funcs...
int Driveryk6000::init()                                     // 控制卡初始化 返回控制卡号 0为初始化失败
{
	m_cardID = yk6000_Open();
	if (m_cardID > 0) {
...
void Driveryk6000::moveTo(int Axis1, int xTarget, int Axis2, int yTarget, int Axis3, int zTarget, int MoveSpeed)			// 3轴联动 单位step
{
	...
	int x_shift =xTarget - yk6000_Get_Command_Pos(m_cardID, X_AXIS);
	int y_shift =yTarget - yk6000_Get_Command_Pos(m_cardID, Y_AXIS);
	int z_shift =zTarget - yk6000_Get_Command_Pos(m_cardID, Z_AXIS);
...
void Driveryk6000::axisMoveVel(MotionAixs axis, int dir, int speed)
{
	if (m_cardID < 0)return;
